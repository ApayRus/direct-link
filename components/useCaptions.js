 ...
 */
import { clearYoutubeAutogenerated, parseSubs } from 'frazy-parser'
...
const getPhrases = (text, languageCode) => {
	let phrases = parseSubs(text)
	if (languageCode?.includes('-auto')) {
		phrases = clearYoutubeAutogenerated(phrases)
	}

	return phrases.map(phrase => {
		const { text } = phrase?.body?.[0] || {}
		return { ...phrase, data: { text } } // additional field {data: text} for wavesurfer regions
	})
}
...
export default function useCaptions({
	...
	const loadCaptions = async languageCode => {
		...
		if (captionTrack) {
			...
			setCaptions(oldState => {
				const phrases = getPhrases(text, languageCode)
	...
	const captionTextEdited = event => {
		...
		const phrases = getPhrases(text, langCode)
